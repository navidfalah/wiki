...
*   **Connectivity**:
    *   Bluetooth Low Energy (BLE) for initial phone setup.
...
*   **Power**: Designed for 2 years of operation on a single CR2032 coin cell battery with hourly readings.
*   **Enclosure**:
...
*   **TeaBuddy Puck**: Another product (from a different company) that uses a BLE kitchen app, mentioned in cross-ticket support scenarios.
*   **[SenseNode](./sensenode.md)**: A competitor product known for its IP67 waterproofing.
*   **Alex**: An individual whose blog previously contained a typo regarding battery type.
*   **Mira Chen**: Firmware, [MeshSync](./meshsync.md), power profiling lead.
*   **Jonah Park**: PCB, sensors, mechanical lead.
...
*   **BLE (Bluetooth Low Energy)**: A wireless technology used for short-range communication and device setup.
...
*   **IP Rating (Ingress Protection)**: A standard defining the level of sealing effectiveness of electrical enclosures against intrusion from foreign bodies (dust, water, etc.). [Nova Widget](./nova-widget.md) is IP54.
*   **CR2032**: A common coin cell battery type used in the [Nova Widget](./nova-widget.md).

## Contradictions

*   **Battery Type**: There was a previous typo in "Alex's blog" mentioning CR2450. This has been corrected; the [Nova Widget](./nova-widget.md) uses a **CR2032** battery.
