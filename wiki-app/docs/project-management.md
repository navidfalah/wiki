...
*   **Aurora Labs Sprint 14 Retro (June 16)**:
    ...
    *   **Action Items**: Implement a contradiction linter for battery claims, publish the power budget spreadsheet, invite 3 more [beta testers](./beta-testing.md) from the homelab forum.
    *   **Shoutout**: Mira Chen fixed a sleep regression bug in two hours.
...
*   **Aurora Labs Sprint 15 Planning (July 10)**:
    ...
    *   **Committed Tasks**: Mira Chen to profile rejoin spikes on an nRF5340 eval board; Jonah Park to update the comparison page for [SenseNode](./sensenode.md) SN-400; an intern to fix the forum scrape CSS selector.
    *   **Stretch Goals**: Develop an [MQTT](./mqtt.md) export CSV sample and design an OTA (Over-The-Air) Update document (not implementation).
...
*   **Aurora Labs Standup (July 1)**:
    *   **Updates**: Mira Chen released 0.3.8 and is monitoring GitHub for rejoin regressions. Jonah Park reported the IP65 gasket vendor quote is still $8k, with the board advising to wait. The wiki compiled with heuristic mode now has over 40 pages.
    *   **Blockers**: Alex's blog battery typo (CR2450 instead of CR2032) is still indexed; the homelab forum scrape parser is broken on nested quotes.
...
*   **TeaBuddy P0**: Android v1.1 beta release, TB-142 cancel bug verification, box copy audit for all presets.
...
*   **MeshSync**:
    ...
    *   Action items include capturing a 24-hour trace on the staging mesh and comparing nRF52840 vs nRF5340 for the next revision.
    *   A wiki page "known mesh quirks v0.3" is planned.
    *   0.3.8 has been released, with 0.3.9 focused on rejoin hardening.
*   **Nova Widget**: Beta boards have arrived, and the pebble-shaped enclosure design was approved. It uses UART provisioning, not consumer QR flow.
*   **SenseNode**: A teardown blog indicates Aurora's IP54 rating is weak, with a target of IP65. A comparison page update for SenseNode SN-400 is a P0 item.
*   **TeaBuddy**:
    *   Android v1.1 beta is a P0 item.
    *   TB-142 cancel bug requires verification.
    *   iOS 18 beta changes to CoreBluetooth pairing caused ticket #2156, requiring `CBManagerAuthorization` to resolve before QR deep link triggers GATT connect.
...
*   **Mira Chen**: Mira Chen Aurora Labs team member, involved in firmware, bug fixes, and sprint planning.
*   **Jonah Park**: Jonah Park Aurora Labs team member, involved in MeshSync, hardware comparisons, and product design.
*   **Alex**: Alex Author of a blog post with incorrect battery information; TeaBuddy team member.
*   **Sam Rivera**: Sam Rivera TeaBuddy author of iOS 18 CoreBluetooth pairing research notes.
...
*   **Agile Sprints**: Agile Sprints Structured periods of work with defined goals, planning, and retrospectives.
...
*   **Firmware Development**: Firmware Development Software embedded in hardware devices, critical for MeshSync functionality.
...
*   **BLE (Bluetooth Low Energy)**: BLE (Bluetooth Low Energy) Wireless technology used for short-range communication, relevant to TeaBuddy's pairing.
*   **iOS 18 / Android Development**: iOS 18 / Android Development Mobile operating system platforms for TeaBuddy applications.
*   **IP Rating (Ingress Protection)**: IP Rating (Ingress Protection) Standard for defining levels of sealing effectiveness of electrical enclosures against intrusion from foreign bodies and moisture.
...
*   **Contradiction Linter**: Contradiction Linter A tool or process to identify conflicting information within documentation or specifications.
...
*   **OTA (Over-The-Air) Update**: OTA (Over-The-Air) Update Wireless delivery of new software or firmware to devices.
...
*   **Current vs. Target IP Rating**:
    **Contradiction:** A SenseNode teardown blog suggests Aurora's current IP54 rating is weak, while Aurora Labs is pursuing an IP65 rating for improved ingress protection, indicating a gap between current product resilience and desired standards (Slack dump, July 1).
