...
| `--web-search` | Step 3 also searches the internet per dirty topic and adds hits as extra `source_type="web"` chunks — off by default, see [35-web-search-enrichment.md](./35-web-search-enrichment.md) |
...
3. If `--web-search` is on: search the internet for each dirty topic (all topics if `--force`) and append the hits as extra `source_type="web"` chunk entries (`main.py`'s `step_synthesize()` → `web_search.augment_grouped_with_web_results()`) — see [35-web-search-enrichment.md](./35-web-search-enrichment.md)
